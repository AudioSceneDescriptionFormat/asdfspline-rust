...
pub trait NewCentripetalKochanekBartels<S: Scalar, V: Vector<S>> {
    // TODO: require alga::linear::NormedSpace to get norm() method for vectors?
    fn new_centripetal_kochanek_bartels<F: Fn(V) -> S>(
        positions: &[V],
        tcb: &[[S; 3]],
        closed: bool,
        get_length: F,
...
impl<S: Scalar, V: Vector<S>> NewCentripetalKochanekBartels<S, V> for PiecewiseCubicCurve<S, V> {}
